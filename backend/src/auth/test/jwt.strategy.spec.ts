...
describe('JwtStrategy.validate', () => {
  it('should reject pre-auth tokens', async () => {
    ...
    await expect(
      strategy.validate({}, { role: 'pre-auth', username: 'admin' }),
  ...
  it('should reject blacklisted tokens', async () => {
    ...
    await expect(
      strategy.validate({}, { username: 'admin', jti: 'x', ver: 0 }),
  ...
  it('should reject tokens with a stale ver claim (credentials changed)', async () => {
    ...
    await expect(
      strategy.validate({}, { username: 'admin', jti: 'x', ver: 1 }),
  ...
  it('should reject legacy tokens without a ver claim', async () => {
    ...
    await expect(
      strategy.validate({}, { username: 'admin', jti: 'x' }),
    ).rejects.toThrow(UnauthorizedException)
  })

  it('should accept tokens with the current ver claim', async () => {
    const strategy = makeStrategy({ tokenVersion: 2 })
    const user = await strategy.validate({}, { sub: 'admin', username: 'admin', jti: 'x', exp: 1, ver: 2 })
