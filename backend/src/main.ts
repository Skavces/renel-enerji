...
import { NtfyLogger } from './notifications/ntfy-logger.service'

async function bootstrap() {
  const app = await NestFactory.create<NestExpressApplication>(AppModule, { rawBody: true, bufferLogs: true })

  // Tüm Logger.error çağrıları ntfy'ye de bildirilir (env yoksa no-op)
  app.useLogger(app.get(NtfyLogger))
